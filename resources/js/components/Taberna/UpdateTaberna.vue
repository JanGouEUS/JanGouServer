...
<template>
    <div class="container" v-bind:style="{ backgroundColor: '#333333',color: '#b5bd2c'}">
            ...
            <div class="col-md-8">
                <div class="card ">
                    ...
                    <div class="card-body dark" v-bind:style="{ backgroundColor: '#535353',color: '#b5bd2c'}">
                        ...
                        <div class="row justify-content-center">
                            <qrcode id="qr"  :value="'https://jangou.eus/Splash/taberna?id='+hash" :options="{ width: 200 }" tag="img" ></qrcode>
                        </div>
                              <!-- <a class="btn btn-success" target="_blank"
                                                v-bind:href="'/pdf/see/'+this.hash"><em
                                                    class="fa fa-download" style="color:white;"></em></a> -->
                        <div class="row justify-content-center mt-1 mb-1">
                            <button type="button" @click="saveQR" class="btn" v-bind:style="{ backgroundColor: '#b5bd2c',color: 'black'}">
                                <em class="fa fa-download" style="color:black;"></em> QR PDF</button>
                        </div>
                        <!-- </div> -->
                        ...
                        <!-- </div> -->
                            ...
                            <div   class="card-footer" v-bind:style="{ backgroundColor: '#333333',color: '#b5bd2c'}">
                                ...
                                <div v-if="mapVisible" id='map' style='width:100%; height: 300px;'></div>
                                <div class="row justify-content-center"> 
                                    <button id="mapa" v-if="!mapVisible" type="button" @click="mapVisible=true"
                                        class="btn  btn-sm" v-bind:style="{ backgroundColor: '#b5bd2c',color: 'black'}">{{$t('createTaberna.map')}}</button>
                                 </div>
                                 <div class="row justify-content-center">    
                        ...
                        <div class="card">
                            ...
                            <div class="card-body" v-bind:style="{ backgroundColor: '#333333',color: '#b5bd2c'}">
                                <div class="form-group">
                            <img v-if="imageLink==''" :src="'https://jangou.eus'+imageLinkURL" width="200px" class="img-responsive">
                            ...
                            <label for="imageLink"> {{$t('createTaberna.imageLink')}}</label>
                             ...
                             <input id="imageLinkInput" type="file" accept="image/*" @change="onFileChangeImageLink" 
                        ...
                        <div class="form-group">
                            <img v-if="logoLink==''" :src="'https://jangou.eus'+logoLinkURL" width="200px" class="img-responsive">
                    ...
                    <div class="modal-dialog" role="document">
                        ...
                        <div class="row justify-content-center">
                            <button type="button" @click="check" class="btn" v-bind:style="{ backgroundColor: '#b5bd2c',color: 'black'}">SEND</button>
                        </div>
...
<script>
    ...
    export default {
        ...
        methods: {
         seePdf() {
             this.qr=document.getElementById('qr').getAttribute('src')
                axios.get(`/pdf/see/${this.hash}/${this.qr}`, {
                        params:{
                            qr:this.qr
                        }
                    })
                    .then(res => {
                        let url = window.URL.createObjectURL(new Blob([res.data.pdf]))
                        ...
                        if (res.data == '200')
                            this.successMsg = 'Pdf-a ondo erakutsi da';
                    })
            },
           saveQR(){
                this.qr=document.getElementById('qr').getAttribute('src')
              this.uploadQR(this.qr,'png','qr',this.hash) 
            ...
            */
            ...
            getPlaceCoordenates() {
                ...
                axios.get(
                    ...
                    console.log(error)
                    this.errorSend('updateTaberna','getPlaceCoordenates',error.response.data.message)
            ...
            */
                ...
                if (this.checkName() && 
                    ...
                    this.uploadImage(this.imageLink,this.imageLinkType,'imageLink',this.hash)
                        if(this.imageLinkURL){
                            this.deleteImage(this.imageLinkURL)
                        }
                    }
                    if(this.logoLink!=''){
                    this.uploadLink(this.logoLink,this.logoLinkType,'logoLink',this.hash)
                        if(this.logoLinkURL){
                            this.deleteImage(this.logoLinkURL)
                        }
                    } 
                    this.updateDescriptions()
                    this.updateTimetables()
                    this.updateMoreInfos()
                    this.updateRequestProcess()
                    this.updatePayProcess()
                    this.updateServicesProcess()
            ...
            checkPlaces() {
                var isGood = true;
                this.placeError = '';
                    if (this.place == null || this.place == ''||this.lat==null || this.lng==null || this.lat=='' || this.lng=='') {
                        this.place=''
            ...
            */
            ...
            preUpload(){
             ...
             uploadQR(image,type,imageFunction,idTaberna){
                axios.post('/image/upload',{
                    ...
                    this.qrURL=response.data.replace('storage', 'public')
                    window.location.href ='/pdf/see/'+this.hash   
                }).catch(error => {
                    this.showModalError()
                    console.log(error)
                    this.errorSend('updateTaberna','uploadQR',error.response.data.message)
                });
            },
            uploadImage(image,type,imageFunction,idTaberna){
              ...
              uploadLink(image,type,imageFunction,idTaberna){
                axios.post('/image/upload',{
                    ...
                    this.showModalError()
                    console.log(error)
                    this.errorSend('updateTaberna','uploadLink',error.response.data.message)

                });
            },
            deleteImage(url){
                url.replace('public','storage')
                axios.post('/image/delete',{
                    ...
                    this.showModalError()
                    console.log(error)
                    this.errorSend('updateTaberna','deleteImage',error.response.data.message)
                });
            },
            ...
            */
            ...
            createRequestMethod(element){       
                axios.post('/request/create',{
                    ...
                    this.showModalError()
                    console.log(error)
                    this.errorSend('updateTaberna','createRequestMethod',error.response.data.message)

                });
             
            },
            deleteRequestMethod(element){       
                axios.post('/request/delete',{
                    id:element,
                }).then(response => {
                    if(response.status==200)
                    this.showToast('success',this.$t('createTaberna.request'))
                    ...
                    this.showModalError()
                    console.log(error)
                    this.errorSend('updateTaberna','deleteRequestMethod',error.response.data.message)
            ...
            updateRequestProcess(){
                 this.requestSelected.forEach(element => {
                    var isIN=false
                    this.requestSelectedOLD.forEach(elementOLD => {
                            if(element ==elementOLD.method){
                                isIN=true
                            }
                    })
                    if(!isIN){
                        this.createRequestMethod(element)
                    }
                 })
                  this.requestSelectedOLD.forEach(element => {
                    var isIN=false
                    this.requestSelected.forEach(elementNEW => {
                            if(element.method ==elementNEW){
                                isIN=true
                            }
                    })
                    if(!isIN){
                        this.deleteRequestMethod(element.id)
                    }
                 })
            },
            /*
            ...
            */
            ...
            createPayMethod(element){
                axios.post('/pay/create',{
                    ...
                    this.showModalError()
                    console.log(error)
                    this.errorSend('updateTaberna','createPayMethod',error.response.data.message)

                });
            },
            deletePayMethod(element){       
                axios.post('/pay/delete',{
                    id:element,
                }).then(response => {
                    if(response.status==200)
                    this.showToast('success',this.$t('createTaberna.pay'))
                    ...
                    this.showModalError()
                    console.log(error)
                    this.errorSend('updateTaberna','deletePayMethod',error.response.data.message)
                });
            ...
            updatePayProcess(){
                 this.payProcedimentsSelected.forEach(element => {
                    var isIN=false
                    this.payProcedimentsSelectedOLD.forEach(elementOLD => {
                            if(element ==elementOLD.method){
                                isIN=true
                            }
                    })
                    if(!isIN){
                        this.createPayMethod(element)
                    }
                 })
                  this.payProcedimentsSelectedOLD.forEach(element => {
                    var isIN=false
                    this.payProcedimentsSelected.forEach(elementNEW => {
                            if(element.method ==elementNEW){
                                isIN=true
                            }
                    })
                    if(!isIN){
                        this.deletePayMethod(element.id)
                    }
                 })
            },
             /*
            ...
            */
              createServices(element){
                axios.post('/service/create',{
                    ...
                    this.showModalError()
                    console.log(error)
                    this.errorSend('updateTaberna','createServices',error.response.data.message)
                });
            },
            deleteServices(element){       
                axios.post('/service/delete',{
                    ...
                    this.showModalError()
                    console.log(error)
                    this.errorSend('updateTaberna','deleteServices',error.response.data.message)
                });
            ...
            updateServicesProcess(){
                 this.servicesSelected.forEach(element => {
                    var isIN=false
                    this.servicesSelectedOLD.forEach(elementOLD => {
                            if(element ==elementOLD.method){
                                isIN=true
                            }
                    })
                    if(!isIN){
                        this.createServices(element)
                    }
                 })
                  this.servicesSelectedOLD.forEach(element => {
                    var isIN=false
                    this.servicesSelected.forEach(elementNEW => {
                            if(element.method ==elementNEW){
                                isIN=true
                            }
                    })
                    if(!isIN){
                        this.deleteServices(element.id)
                    }
                 })
            },
            ...
            */
            ...
            updateTaberna(){
                axios.post('/taberna/update',{
                    ...
                    this.hideLoadingModal()
                    this.showModalError()
                     this.errorSend('updateTaberna','updateTaberna',error.response.data.message)
           ...
           updateDescriptions(){
              this.languagesSelected.forEach(element => {  
                axios.post('/description/update',{
                    ...
                    this.showModalError()
                    console.log(error)
                    this.errorSend('updateTaberna','updateDescriptions',error.response.data.message)
                });
              })
            },
            /*
            ...
            updateTimetables(){
              this.languagesSelected.forEach(element => {   
                axios.post('/timetable/update',{
                    ...
                    this.showModalError()
                    console.log(error)
                    this.errorSend('updateTaberna','updateTimetables',error.response.data.message)
                });
              })
            },
            ...
            */
            updateMoreInfos(){
              this.languagesSelected.forEach(element => { 
                  if (this.moreInfos[element] == null || this.moreInfos[element] == ''){
                        console.log('NO MoreInfo ',element)
                  } else{ 
                      if(this.moreInfosOLDtoUpdate[element] == null || this.moreInfosOLDtoUpdate[element] == ''){
                            axios.post('/moreinfo/create',{
                                ...
                                this.showModalError()
                                console.log(error)
                                this.errorSend('updateTaberna','updateMoreInfosCreate',error.response.data.message)

                            });
                        }else{
                             axios.post('/moreinfo/update',{
                                idTaberna:this.hash,
                                language:element,
                                moreInfo:this.moreInfos[element]
                            }).then(response => {
                                ...
                                this.showModalError()
                                console.log(error)
                                this.errorSend('updateTaberna','updateMoreInfosUpdate',error.response.data.message)
        ...
        getTaberna(){
                axios.get('/taberna/get/taberna').then(response => {
                    this.hash=response.data.Taberna[0].idTaberna
                    this.name=response.data.Taberna[0].name
                    this.place= response.data.Taberna[0].address
                    this.lat=response.data.Taberna[0].latitude
                    this.lng=response.data.Taberna[0].longitude
                    this.telephone=response.data.Taberna[0].telephone
                    this.email=response.data.Taberna[0].email
                    this.web=response.data.Taberna[0].web
                    this.twitter=response.data.Taberna[0].twitter
                    this.facebook=response.data.Taberna[0].facebook
                    this.instagram=response.data.Taberna[0].instagram
                    this.telegram=response.data.Taberna[0].telegram
                    this.imageLinkURL=response.data.Taberna[0].imagelink
                    this.logoLinkURL=response.data.Taberna[0].logolink
            ...
            getDescriptions(){
                axios.get('/description/get',{
                    params:{
                        idTaberna:this.hash
                    }
                }).then(response => {
                    this.descriptionsOLD=response.data.Descriptions
                    response.data.Descriptions.forEach(element => { 
                        this.descriptions[element.language]=element.description 
                    })
                    this.languageSelect='es'
                    this.languageSelect='eu'
                }).catch(error => {
                    console.log(error)
                    this.errorSend('updateTaberna','getDescriptions',error.response.data.message)

                });
            },
            getTimetables(){
                axios.get('/timetable/get',{
                    params:{
                        idTaberna:this.hash
                    }
                }).then(response => {
                    this.timetablesOLD=response.data.Timetables
                    response.data.Timetables.forEach(element => { 
                        this.timetables[element.language]=element.timeTable 
                    })
                    this.languageSelect='es'
                    this.languageSelect='eu'
                }).catch(error => {
                    console.log(error)
                    this.errorSend('updateTaberna','getTimetables',error.response.data.message)

                });
            },
             getMoreinfos(){
                axios.get('/moreinfo/get',{
                    params:{
                        idTaberna:this.hash
                    }
                }).then(response => {
                    this.moreInfosOLD=response.data.MoreInfo
                    response.data.MoreInfo.forEach(element => { 
                        this.moreInfos[element.language]=element.moreInfo 
                        this.moreInfosOLDtoUpdate[element.language]=element.moreInfo
                    })
                    this.languageSelect='es'
                    this.languageSelect='eu'
                }).catch(error => {
            ...
            getRequests(){
                axios.get('/request/get',{
                    params:{
                        idTaberna:this.hash
                    }
                }).then(response => {
                    this.requestSelectedOLD=response.data.Requests
                    response.data.Requests.forEach(element => { 
                        if(element.method!='Telephone'&&
                            element.method!='Whatsapp'&&
                            element.method!='Telegram'&&
                            element.method!='Email')
                        {
                            this.newRequestLinks.push(element.method) 
                        }
                        
                        this.requestSelected.push(element.method) 
                    })
                }).catch(error => {
                    console.log(error)
                    this.errorSend('updateTaberna','getRequests',error.response.data.message)
                });
            },
            getPays(){
                axios.get('/pay/get',{
                    ...
                    this.payProcedimentsSelectedOLD=response.data.Pays
                    response.data.Pays.forEach(element => { 
                        if(element.method!='Card'&&
                            element.method!='Coin'&&
                            element.method!='Bizum')
                        {
                            this.newPayProcediments.push(element.method) 
                        }                      
                        this.payProcedimentsSelected.push(element.method) 
                    })
                }).catch(error => {
                    console.log(error)
                    this.errorSend('updateTaberna','getPays',error.response.data.message)
            ...
            getServices(){
                axios.get('/service/get',{
                    ...
                    this.servicesSelectedOLD=response.data.Services
                    response.data.Services.forEach(element => {                      
                        this.servicesSelected.push(element.services) 
                    })
                     this.qr=document.getElementById('qr').getAttribute('src')
                }).catch(error => {
                    console.log(error)
                    this.errorSend('updateTaberna','getServices',error.response.data.message)
